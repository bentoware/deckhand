...
COMMAND_CATALOG: tuple[CommandCatalogEntry, ...] = (
    ...
    _entry("anki.webengine.click", "dev_exec", "Click in an Anki Qt WebEngine CDP page by latest snapshot uid, selector, visible text, or coordinates; this changes UI state." + WEBENGINE_TARGET_NOTE, status="implemented", input_schema=_schema({**CDP_TARGET, "uid": {"type": "string", "description": "Element uid from the latest take_snapshot result."}, "selector": {"type": "string"}, "text": {"type": "string"}, "x": {"type": "number"}, "y": {"type": "number"}, "button": {"type": "string", "enum": ["left", "middle", "right", "none"]}, "clickCount": {"type": "integer", "minimum": 1}})),
    _entry("anki.webengine.type_text", "dev_exec", "Type text into an Anki Qt WebEngine CDP page, optionally focusing by latest snapshot uid or selector first; this changes UI state." + WEBENGINE_TARGET_NOTE, status="implemented", input_schema=_schema({**CDP_TARGET, "text": {"type": "string", "minLength": 1}, "uid": {"type": "string", "description": "Element uid from the latest take_snapshot result."}, "selector": {"type": "string"}, "clear": {"type": "boolean"}}, ["text"])),
